...
module.exports = async function(client, button) {
        try {
            ...
            if (button.id == 'createTicket') {
                ...
                let msgNya = require('quick.db').fetch(`tickMsg_${button.guild.id}`)
                if(msgNya == null) {
                  msgNya = ee.defaultOpenMsg;
                }
                let cate = require('quick.db').fetch(`tickCat_${button.guild.id}`)
                if(cate == null) {
                  cate = button.channel.parentID;
                }
                ...
                if (checkTickets) {
                   ...
                   ephemeral: true }).then(async function(m) {
                        setTimeout(() => {
                            m.delete();
                        }, 1000 * 7);
                    });
...
.setLabel("")
                   ...
                   //     .addComponent(btn2)
                    ...
                    let tn = require('quick.db').fetch(`TicketTerbuka`)
                    if(tn == null) {
                      tn = '0';
                    }
                    let tp = tn + 1;
                    require('quick.db').set(`TicketTerbuka`, tp)
                    });
      ...
      .setEmoji('887650847327158343')
                ...
                button.message.reply({
                 ...
                 setTimeout(() => {
                   ...
                   let ddy = require('quick.db').fetch(`TicketTerbuka`);
if(ddy >= 0 && ddy != null) {
let ddi = ddy - 1;
require('quick.db').set(`TicketTerbuka`, ddi);
}
                }, 1000 * 4.3);
